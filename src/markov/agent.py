...
class Agent:
    ...
    def __init__(
        ...
        self.states_encoder: Dict[State, int] = {s: idx for idx, s in enumerate(states)}
        self.states_decoder: Dict[int, State] = {idx: s for idx, s in enumerate(states)}
        self.actions_encoder: Dict[Action, int] = {a: idx for idx, a in enumerate(actions)}
        self.actions_decoder: Dict[int, Action] = {idx: a for idx, a in enumerate(actions)}
        self.rewards_encoder: Dict[Reward, int] = {r: idx for idx, r in enumerate(rewards)}
        self.rewards_decoder: Dict[int, Reward] = {idx: r for idx, r in enumerate(rewards)}

        self.states_actions = itertools.product(self.states_decoder.keys(), self.actions_decoder.keys())
        self.history = []
        self.total_counts = 0
    ...
    def __update(self, state: State, reward: Reward) -> None:
        ...
        self.total_counts += 1
        # update probs
        self.probs[(idx_last_state, idx_last_action)] = self.counts[(idx_last_state, idx_last_action)] / self.total_counts
